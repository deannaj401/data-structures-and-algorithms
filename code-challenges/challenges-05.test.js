...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(data => {
    ...
    $character.find('h2').text(data.name);
    $character.find('h3').text(data.height);
    $character.find('p').text(data.eye_color);
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(data => {
    //find index of the 2nd space in the str
    let foodItem = data.indexOf(" ", data.indexOf(" ") + 1);
    //slice after the 2nd space and push
    result.push(data.slice(foodItem + 1));
  });
  return result;
};
