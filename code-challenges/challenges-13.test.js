...
const findHappiness = (arr) => {
  let happyFace = [];
  arr.forEach(happy => {
    if (happy.includes(':)') && happy.includes(':)') ) {
      //console.log('smiles', smiles);
      happyFace.push(happy);
    }
  });
  return happyFace;
};
...
const standardizePhoneNumbers = (arr) => {
  let phoneNumb = arr.map(num => {
    let beginning = num.substring(1, 4);
    let middle = num.substring(6, 9);
    let end = num.substring(10, 14);
